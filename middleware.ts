...
export const config = {
  matcher: [
    ...
    // `share` = public tokenized report pages + their data endpoint —
    // the token IS the credential there.
    "/((?!api/auth|api/health|api/thumb|api/share|api/sync/trigger|api/sync/ingest|api/sync/ig-missing-data|share/|_next/static|_next/image|favicon\\.ico|.*\\.(?:png|jpg|jpeg|gif|svg|webp|ico|woff|woff2|ttf)$).*)",
  ],
};
