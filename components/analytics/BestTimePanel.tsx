...
/**
 * "Timing" view of the Posts page — best time to post, computed from
 * the workspace's OWN history: every published post bucketed by
 * (weekday × 2-hour block) in the viewer's local timezone, slots
 * compared by median lifetime views. Runs its own lookback-window
 * fetch (independent of the header date picker) and shares the
 * page-level tag filter passed in via `tags`.
 */
export default function BestTimePanel({ tags }: { tags: string[] }) {
        ...
        <>
          ...
          <div style={{ background: "var(--bg-elev)", border: "1px solid var(--border)", borderRadius: 14, padding: 20 }}>
            ...
            <div style={{ fontSize: 11, color: "var(--fg-subtle)", marginBottom: 14 }}>
              {sampleSize.toLocaleString()} posts analyzed{platform !== "all" ? ` on ${PLATFORM_LABEL[platform] ?? platform}` : ""} · times in your timezone · gray = fewer than {MIN_POSTS} posts in slot
            </div>
